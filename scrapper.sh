...
then
    ...
    then
		for docset in angular apache backbone bower browserify c chai clojure coffeescript cordova cpp css d3 django dom dom_events drupal ember express git go grunt html http haskell iojs jasmine jquery_core jquery_ui jquery_mobile javascript knockout laravel less lodash lua marionette markdown mocha modernizr moment mongoose nginx node nokogiri2 npm opentsdb phalcon phaser php2 phpunit postgresql python2 python q rails react react_native react_router redis reflux requirejs rethinkdb ruby rust sass sinon socketio svg symfony underscore vue webpack xpath yii yii1
		do
